...
$(document).on("click", ".darkmode", function(){
	...
	buttonenabled = false;
	$(".clip").html($("body #root #container")[0].outerHTML); //Copy container to inside clip
	scrollbind($(".clip .container"));
	$(".clip .container").toggleClass("dark").scrollTop(scroll); //Toggle dark mode and set scroll
	$(".darkmode").toggleClass("fa-moon").toggleClass("fa-sun"); //Make changes: change button icon
	$(".clip").addClass("anim"); //Animate the clip
	setTimeout(function(){
		// $("body #root #container").replaceWith($(".clip").html()) //Replace container with clip html
	
		$(".container").toggleClass("dark").scrollTop(scroll); //Toggle dark mode and set scroll
		$(".darkmode").toggleClass("fa-sun").toggleClass("fa-moon"); //Make changes: change button icon
		$(".clip").html("");
		scrollbind($("body #root #container")); //bind scroll with new container
		$("body #root #container").scrollTop(scroll); //Set scroll position
