...
import tableStyle from "../AdminStyle/tableStyle.module.css";
    ...
    const headCells =[{id:"moduleAbrv",label:"abreveation"}, {id:"nomModule",label:"Module"}, {id:"coefficient",label:"coefficient"}, {id:"niveau",label:"Level"}, {id:"hasTDTP",label:"type"},{id:"actions",label:"actions"}]
        ...
        <div className={tableStyle.container}>
            <div className={tableStyle.content_box + ' '+tableStyle.special}>
                <div className={tableStyle.pageHeader__version2}>
                    <h1>Modules list :</h1>
                    <a href="addModule" className={tableStyle.btn + " "+ tableStyle.cta_btn}>+ Add New Module</a>
                </div>
                                    ...
                                    <TableBody>
                                        {
                                            ...
                                            .map((module,index) => (
                                                <StyledTableRow hover role="checkbox" tabIndex={-1} key={index} >
                                                    ...
                                                    <StyledTableCell component="th" scope="row" >{module.moduleAbrv}</StyledTableCell>
                                                    <StyledTableCell >{module.nomModule}</StyledTableCell>
                                                    <StyledTableCell >{module.coefficient}</StyledTableCell>
                                                    <StyledTableCell >{module.niveau}</StyledTableCell>
                                                    <StyledTableCell >{module.hasTDTP ? "Cours + TD/TP" : "COURS only"}</StyledTableCell>
                                                    <StyledTableCell >
                                                        <div style={{display:"flex",flexDirection:"row",justifyContent:"space-evenly"}}>
                                                            <IconButton sx={{border: "1px solid #005c75", boxShadow:" 2px 2px 5px rgba(0, 0, 0, .3)"}}
                                                               onClick={()=>goTo(`/module/editModule/${module.id}`)} 
                                                            >
                                                                <EditIcon />
                                                            </IconButton>
                                                            <IconButton sx={{border: "1px solid #005c75", boxShadow:" 2px 2px 5px rgba(0, 0, 0, .3)"}}
