...
const headCells =[{id:"userId",label:"card number"}, {id:"lastName",label:"last Name"}, {id:"firstName",label:"first Name"}, {id:"email",label:"email"}, {id:"groupe",label:"group"}, {id:"section",label:"section"}, {id:"niveau",label:"level"},{id:"actions",label:"actions"}]
...
const ListEtudiants = () => {
    ...
    const getAllEtudiants = () =>{
        EtudiantService.getAllEtudiants().then((Response)=>{
    ...
    const deleteEtudiant = (userId) =>{
        console.log(userId);
        EtudiantService.deleteEtudiant(userId).then((Response)=>{
            getAllEtudiants();
        }).catch(Error=>{
            console.log(Error)
        })
        ...
        <div className={tableStyle.container}>
            <div className={tableStyle.content_box + ' '+tableStyle.special}>
                <div className={tableStyle.pageHeader__version2}>
                    <h1>Students list :</h1>
                    <a href="/admin/addEtudiant" className={tableStyle.btn + " "+ tableStyle.cta_btn}>+ Add New Student</a>
                </div>
                        ...
                        <Paper elevation={4} sx={{ width: '100%', overflow: 'hidden' , border:"1px solid #014055" }}>
                            <TableContainer sx={{ height: 550 }}>
                                <Table stickyHeader sx={{ minWidth: 700}} aria-label="sticky table">
                                    ...
                                    <TableBody>
                                        {
                                            stableSort(etudiants, getComparator(order, orderBy))
                                            .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                                            .map((etudiant,index) => (
                                                <StyledTableRow hover role="checkbox" tabIndex={-1} key={index} >
                                        
                                                    <StyledTableCell component="th" scope="row" >{etudiant.useId}</StyledTableCell>
                                                    <StyledTableCell >{etudiant.lastName}</StyledTableCell>
                                                    <StyledTableCell >{etudiant.name}</StyledTableCell>
                                                    <StyledTableCell >{etudiant.email}</StyledTableCell>
                                                    <StyledTableCell >{etudiant.groupe}</StyledTableCell>
                                                    <StyledTableCell >{etudiant.section}</StyledTableCell>
                                                    <StyledTableCell >{etudiant.niveau}</StyledTableCell>
                                                    <StyledTableCell >
                                                        <div style={{display:"flex",flexDirection:"row",justifyContent:"space-evenly"}}>
                                                            <IconButton sx={{border: "1px solid #005c75", boxShadow:" 2px 2px 5px rgba(0, 0, 0, .3)"}}
                                                                onClick={()=>goTo(`/admin/editEtudiant/${etudiant.userId}`)}
                                                            >
                                                                <EditIcon />
                                                            </IconButton>
                                                            <IconButton sx={{border: "1px solid #005c75", boxShadow:" 2px 2px 5px rgba(0, 0, 0, .3)"}}
